...
If you're in a rush, skip down to the \*\*\*\*\*\*\*\*\*\*: this section is about using your own data.
...
Oh wait, that's not right. If we start the name of the output with a number, R gets confused.

`OnemgVC <- filter(ToothGrowth, supp == 'VC', dose == 1.0)`

![]( /R101imgs/r19.PNG)

Now that subset of data is available to view on its own.
...
The p-value is very high, 0.779. In the context of a one-sample t-test, the p-value being **over** the significance level (usually 0.05, but it varies) indicates **support for the null hypothesis**.

Therefore, the vets' claims about 1g of VC producing 17mm teeth is supported by these data!
